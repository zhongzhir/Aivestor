...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  const guard = await requireCapabilityAPI("zjjr_data");
  if (!guard.ok) return guard.response;
  const rows = await query(`DELETE FROM intelligence_tasks WHERE id=$1 AND user_id=$2 RETURNING id`, [params.id, guard.ctx.userId]);
  if (!rows[0]) return NextResponse.json({ error: "任务不存在" }, { status: 404 });
