...
export async function GET() {
  ...
  const tasks = await query(`SELECT * FROM intelligence_tasks WHERE user_id = $1 ORDER BY updated_at DESC`, [guard.ctx.userId]);
  const briefs = await query(`SELECT id, task_id, task_name, coverage_start, coverage_end, generated_at, item_count, important_facts, trend_signals, other_items, source_list FROM intelligence_briefs WHERE user_id = $1 ORDER BY generated_at DESC LIMIT 10`, [guard.ctx.userId]);
  return NextResponse.json({ tasks, briefs });
}
