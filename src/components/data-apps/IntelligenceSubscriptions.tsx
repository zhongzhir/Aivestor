...
export default function IntelligenceSubscriptions() {
  ...
  async function parseDescription() {
    ...
    const response = await fetch("/api/data-apps/intelligence-subscriptions/parse", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ description }) });
  ...
  async function action(task: Task, kind: "generate" | "delete" | "toggle") {
    ...
    if (!response.ok) { const data = await response.json().catch(() => ({})); setError(friendlyError(data.error ?? "")); }
