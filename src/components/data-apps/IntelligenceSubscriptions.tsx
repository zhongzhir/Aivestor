...
export default function IntelligenceSubscriptions() {
  ...
  async function parseDescription() {
    ...
    try {
      ...
      if (!response.ok || !data.plan) { setError(responseError(response.status, data.error, "请再补充关注对象或时间节奏，然后重试。")); return; }
      setPlan(data.plan);
    } catch {
  ...
  async function confirmPlan() {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) { if (response.status === 402 || data.code === "quota_unavailable") setQuotaBlocked(true); setError(responseError(response.status, data.error, friendlyError(data.error ?? ""))); return; }
      setDescription(""); setPlan(null); setNotice("任务已创建，可以在下方任务列表中生成本期简报。 ");
  ...
  async function saveAdvanced(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setShowForm(false); setEditing(null); setAdvancedSeed(null); setPlan(null); setNotice(editing ? "任务设置已保存。" : "任务已创建，可以在下方任务列表中生成本期简报。 ");
  ...
  return <div>
    ...
    <section className="mt-6 rounded-lg border border-line bg-white p-5 sm:p-7">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3"><button onClick={parseDescription} disabled={busy} className="rounded-md bg-[#0D1B3E] px-4 py-2 text-sm text-white disabled:opacity-50">{busy ? "正在理解…" : "理解我的需求"}</button><button onClick={() => openAdvanced()} className="px-2 py-2 text-sm text-ink-faint hover:text-ink">需要更多控制？高级设置</button></div>
