...
export default function IntelligenceSubscriptions() {
  ...
  return <div>
    ...
    <section className="mt-7"><h2 className="text-base font-semibold">过去生成的简报</h2>{briefs.length === 0 ? <p className="mt-3 text-sm text-ink-faint">生成后的简报会保留在这里，方便回看。</p> : <div className="mt-3 space-y-5">{briefs.map((brief) => <div key={brief.id} className="rounded-xl border border-line bg-white p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3 border-b border-line pb-4"><div><p className="text-base font-semibold text-ink">{brief.task_name}</p><p className="mt-1 text-xs text-ink-faint">覆盖 {new Date(brief.coverage_start).toLocaleDateString("zh-CN")} 至 {new Date(brief.coverage_end).toLocaleDateString("zh-CN")} · 生成于 {new Date(brief.generated_at).toLocaleString("zh-CN")}</p></div><span className="rounded-full bg-[#f1f5fb] px-2.5 py-1 text-xs text-ink-soft">共 {brief.item_count} 条</span></div>{brief.metadata?.overview && <div className="mt-4 rounded-lg bg-[#f8fbff] px-4 py-3 text-sm leading-6 text-ink-soft"><p className="text-xs font-medium text-ink">本期概览</p><p className="mt-1">{brief.metadata.overview}</p></div>}<div className="mt-4 grid gap-4 sm:grid-cols-2">{brief.important_facts?.length > 0 && <BriefSection title="重要事实" tone="blue" items={brief.important_facts} briefId={brief.id} feedback={feedback} onFeedback={markFeedback} />}{brief.trend_signals?.length > 0 && <BriefSection title="趋势信号" tone="amber" items={brief.trend_signals} briefId={brief.id} feedback={feedback} onFeedback={markFeedback} />}{brief.other_items?.length > 0 && <BriefSection title="其他相关信息" tone="gray" items={brief.other_items} briefId={brief.id} feedback={feedback} onFeedback={markFeedback} />}</div>{brief.source_list?.length > 0 && <div className="mt-5 border-t border-line pt-4"><p className="text-xs font-medium text-ink-soft">本期信息源</p><div className="mt-2 flex flex-wrap gap-2">{uniqueSources(brief.source_list).map((source: any) => <a key={`${source.source}-${source.url ?? ""}`} href={source.url ?? undefined} target={source.url ? "_blank" : undefined} rel={source.url ? "noreferrer" : undefined} className="rounded-full border border-line px-2.5 py-1 text-xs text-ink-soft hover:border-[#0D1B3E]">{source.source}</a>)}</div></div>}</div>)}</div>}</section>
  </div>;
}
...
function BriefSection({ title, tone, items, briefId, feedback, onFeedback }: { title: string; tone: "blue" | "amber" | "gray"; items: any[]; briefId: string; feedback: Record<string, string>; onFeedback: (briefId: string, itemKey: string, value: "valuable" | "irrelevant") => void }) {
  ...
  return <section className={`rounded-lg border p-3 ${toneClass}`}><div className="flex items-center justify-between gap-2"><h3 className="text-sm font-semibold text-ink">{title}</h3><span className="text-xs text-ink-faint">{items.length} 条</span></div><div className="mt-2 space-y-3">{items.map((item: any) => <article key={item.id} className="rounded-md border border-white/80 bg-white p-3 shadow-sm"><h4 className="text-sm font-medium leading-6 text-ink">{item.title}</h4><div className="prose prose-sm mt-2 max-w-none text-ink-soft prose-p:my-2 prose-p:leading-6 prose-headings:mb-2 prose-headings:mt-3 prose-headings:text-sm prose-li:my-0.5 prose-ul:my-2"><ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]}>{item.content ?? "暂无摘要"}</ReactMarkdown></div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-line pt-2"><p className="text-xs text-ink-faint">来源：{item.sourceUrl ? <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="text-accent hover:underline">{item.source}</a> : item.source} · {new Date(item.publishedAt).toLocaleString("zh-CN")}</p><div className="flex gap-2 text-xs"><button onClick={() => onFeedback(briefId, item.id, "valuable")} className={feedback[`${briefId}:${item.id}`] === "valuable" ? "font-semibold text-accent" : "text-ink-faint"}>有价值</button><button onClick={() => onFeedback(briefId, item.id, "irrelevant")} className={feedback[`${briefId}:${item.id}`] === "irrelevant" ? "font-semibold text-red-600" : "text-ink-faint"}>无关</button></div></div></article>)}</div></section>;
}
